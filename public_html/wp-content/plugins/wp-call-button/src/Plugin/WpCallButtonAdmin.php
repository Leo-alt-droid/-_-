...
class WpCallButtonAdmin {
	...
	public function init() {
		...
		register_activation_hook( WP_CALL_BUTTON_FILE, [ $this, 'activation_hook' ] );

		// Setup the settings page.
		add_action( 'admin_menu', [ $this, 'settings_menu' ] );

		// Link to settings page.
		add_filter( 'plugin_action_links_' . plugin_basename( WP_CALL_BUTTON_FILE ), [ $this, 'add_settings_link' ], 10, 2 );

		// Admin notices.
		add_action( 'admin_notices', [ $this, 'dashboard_notices' ] );

		// Dismiss welcome notice ajax.
		add_action( 'wp_ajax_' . $this->plugin_slug . '_dismiss_dashboard_notices', [ $this, 'dismiss_dashboard_notices' ] );

		// Ajax endpoint for searching posts / pages / cpts by title.
		add_action( 'wp_ajax_' . $this->plugin_slug . '_get_posts', [ $this, 'get_posts_for_ajax' ] );
	...
	function get_posts_for_ajax() {
		...
		if (
			 ...
			 */
			$post_types = apply_filters( 'wpcb_button_post_types', [ 'post', 'page' ] );
	...
	public function settings_menu() {
		...
		$this->menu = add_options_page( $this->plugin_name_menu, $this->plugin_name, 'administrator', $this->plugin_slug, [ $this, 'settings_menu_cb' ] );

		// Load admin styles based on the hook suffix.
		if ( $this->menu ) {
			add_action( 'load-' . $this->menu, [ $this, 'admin_assets' ] );
		}
	}
				...
				if ( isset( $_REQUEST['submit'] ) && isset( $_REQUEST['action'] ) && 'update' === $_REQUEST['action'] ) {
					...
					$settings = [
						...
						'wpcallbtn_button_filter_ids_show' =>
							...
							array_map( 'sanitize_text_field', wp_unslash( $_POST['wpcallbtn_button_filter_ids_show'] ) ) : [],
						'wpcallbtn_button_filter_ids_hide' =>
							...
							array_map( 'sanitize_text_field', wp_unslash( $_POST['wpcallbtn_button_filter_ids_hide'] ) ) : [],
